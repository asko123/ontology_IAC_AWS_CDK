...
## ML Stack (External Resource)

### SageMaker Endpoint

**Purpose**: Generate vector embeddings for text chunks using Hugging Face models

**Note**: SageMaker endpoint is **deployed separately** before CDK deployment. It's not part of the CDK stacks but is referenced by the Embedding Generator Lambda.
...
- **Instance**: ml.m5.large
  - vCPUs: 2
  - Memory: 8 GB
  - Cost: ~$0.115/hour = **$84/month** (24/7)
- **Auto-scaling**: Optional (can add)
- **Availability**: Single AZ (can configure multi-AZ)

**Deployment Details**:
- **Runtime**: Hugging Face Inference Container
- **Embedding Dimensions**: 768
- **Max Sequence Length**: 512 tokens
- **Batch Size**: Configurable (default: 8-16)

**Instance Types**:
| Type | vCPUs | Memory | Cost/Hour | Cost/Month (24/7) | Use Case |
|------|-------|--------|-----------|-------------------|----------|
| ml.t3.medium | 2 | 4 GB | $0.05 | $36 | Dev/Testing |
| ml.m5.large | 2 | 8 GB | $0.115 | $84 | **Current (Production)** |
| ml.m5.xlarge | 4 | 16 GB | $0.23 | $168 | High throughput |
| ml.c5.xlarge | 4 | 8 GB | $0.204 | $149 | Compute-optimized |
| ml.g4dn.xlarge | 4 | 16 GB + GPU | $0.736 | $537 | GPU acceleration |

**Cost Optimization Options**:

1. **Use Serverless Inference** (if low usage):
   - Pay per invocation ($0.000025 per MB/second)
   - No idle costs
   - Cold start latency: 1-5 seconds
   - Best for: < 1000 requests/day

2. **Use Auto-scaling**:
   ```python
   # Scale based on invocations
   variant.apply_auto_scaling(
       min_capacity=1,
       max_capacity=3,
       target_value=1000  # Invocations per minute
   )
   ```

3. **Use Smaller Instance** (ml.t3.medium):
   - $36/month vs $84/month
   - Suitable for: < 100 docs/day
   - Trade-off: Slower inference

4. **Use Asynchronous Inference**:
   - Queue requests when endpoint is busy
   - Scale down during idle periods
   - Reduce instance count

**Performance Characteristics**:
| Instance | Throughput | Latency (p50) | Cost Efficiency |
|----------|-----------|---------------|-----------------|
| ml.t3.medium | ~50 chunks/min | 500ms | Best for low volume |
| ml.m5.large | ~200 chunks/min | 150ms | **Balanced** |
| ml.m5.xlarge | ~400 chunks/min | 100ms | High volume |
| ml.c5.xlarge | ~350 chunks/min | 120ms | Compute-heavy |

**Monthly Cost Examples**:
```
Scenario 1: Dev/Testing (100 docs/month)
- ml.t3.medium: $36/month
- Total embeddings: ~1,000 chunks
- Cost per chunk: $0.036

Scenario 2: Production (1,000 docs/month)
- ml.m5.large: $84/month
- Total embeddings: ~10,000 chunks
- Cost per chunk: $0.0084

Scenario 3: High Volume (10,000 docs/month)
- ml.m5.xlarge + auto-scaling: $168-$500/month
- Total embeddings: ~100,000 chunks
- Cost per chunk: $0.0017-$0.005
```

**Lambda Integration**:
```typescript
// Embedding Generator Lambda references SageMaker
environment: {
  SAGEMAKER_ENDPOINT_NAME: process.env.SAGEMAKER_ENDPOINT_NAME || 'huggingface-embeddings',
...
**Setup Instructions**:
See [SAGEMAKER_SETUP.md](SAGEMAKER_SETUP.md) for deployment steps.

**Monitoring**:
- **CloudWatch Metrics**: Invocations, ModelLatency, CPUUtilization
- **Alarms**: Set on ModelLatency > 1000ms or 5XX errors > 10
- **Logging**: Endpoint logs available in CloudWatch
