...
**Note**: SageMaker is the second-largest cost ($84/month) after data stores.

```typescript
// Option 1: Use smaller instance for dev/testing
// ml.t3.medium: $36/month (saves $48/month)
SAGEMAKER_ENDPOINT_NAME=huggingface-embeddings-dev

// Option 2: Serverless Inference (low volume)
// Pay per invocation: ~$0.20-$10/month for < 1000 docs
// No idle costs, but has cold start latency

// Option 3: Auto-scaling (variable load)
variant.apply_auto_scaling(
  min_capacity=1,    // Scale to 1 during idle
  max_capacity=3,    // Scale up during peaks
...
```
ml.t3.medium (24/7):     $36/month   ← Best for consistent workload
ml.m5.large (24/7):      $84/month   ← Current (faster inference)
Serverless:              $5-10/month ← Best for sporadic usage
```

**Implementation**:
- See [SAGEMAKER_SETUP.md](SAGEMAKER_SETUP.md) for deployment options
- Configure endpoint name in `.env`
- Monitor usage via CloudWatch to right-size

### 7. Reserved Capacity (Future)
...
**Savings Potential**:
- Neptune Reserved: 40-60% savings on $70/month
- OpenSearch Reserved: 30-45% savings on $60/month
- SageMaker Savings Plans: 20-64% savings on $84/month
